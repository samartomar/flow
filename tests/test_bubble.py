...
class TestTheSettingsStrip(unittest.TestCase):
    """The controls worth reaching without a right-click.

    Asked for by name — "Dictate and Converse for sure Then workspace and voices" — and
    the split between them is the design: mode is a *control*, because it changes what
    Send does and is switched mid-task; workspace and voice are *values*, whose worth is
    being visible, and which open the menu that already exists rather than growing a
    second implementation of the same list.
    """

    def strip(self, **session):
        canvas = MeasuringCanvas()
        pill = mock.Mock()
        pill.session = mock.Mock(**{"mode": ui.DICTATE, "workspace": "",
                                    "speaker": None, "muted": False, **session})
        took = ui._settings_row(canvas, pill, ui.BUBBLE_W, ui.PAD)
        texts = [i["text"] for i in canvas.items if i.get("text")]
        return took, texts, canvas, pill

    def test_the_mode_is_named_and_is_a_chip(self):
        _took, texts, _c, _p = self.strip()
        self.assertTrue(any(t.startswith("Dictate") for t in texts), texts)

    def test_converse_says_converse(self):
        _took, texts, _c, _p = self.strip(mode=ui.CONVERSE)
        self.assertTrue(any(t.startswith("Converse") for t in texts), texts)

    def test_clicking_the_mode_switches_it(self):
        _took, _texts, canvas, pill = self.strip()
        fire = next(fn for tag, seq, fn in canvas.bindings
                    if tag == "settings-mode" and seq == "<Button-1>")
        fire(None)
        pill.session.toggle_mode.assert_called_once()

    def test_a_value_opens_the_menu_that_already_exists(self):
        # Rather than a second implementation of the same list: the workspace recents
        # and the voice list are both built with a tick showing the current choice, and
        # two of anything is two things to keep in step.
        _took, _texts, canvas, pill = self.strip(workspace=r"D:/dev/products/flow")
        fire = next(fn for tag, seq, fn in canvas.bindings
                    if tag == "settings-workshop" and seq == "<Button-1>")
        fire(None)
        pill._menu_workspace.assert_called_once()

    def test_the_workspace_is_shown_by_its_last_component(self):
        # The tail of a path names the project; the head is the part everybody shares.
        for path in ("D:/dev/products/flow", "D:" + chr(92) + "dev" + chr(92) + "flow",
                     "/home/sam/flow"):
            with self.subTest(path=path):
                _took, texts, _c, _p = self.strip(workspace=path)
                self.assertIn("workshop: flow", texts)

    def test_a_workspace_that_is_not_set_takes_no_room(self):
        _took, texts, _c, _p = self.strip(workspace="")
        self.assertFalse([t for t in texts if t.startswith("workshop")])

    def test_the_voice_is_absent_when_nothing_speaks(self):
        _took, texts, _c, _p = self.strip(speaker=None)
        self.assertFalse([t for t in texts if t.startswith("voice")])

    def test_and_says_muted_when_it_is(self):
        _took, texts, _c, _p = self.strip(speaker=mock.Mock(name="x"), muted=True)
        self.assertIn("voice: muted", texts)

    def test_it_takes_the_height_the_layout_reserved_for_it(self):
        # `around` in `_render` counts `SETTINGS_H`; a strip that took more would be
        # drawn over the first line of the draft.
        took, _texts, _c, _p = self.strip()
        self.assertEqual(took, ui.SETTINGS_H)

    def test_a_pill_with_no_session_draws_nothing(self):
        # Fixtures build pills with `__new__`, and a strip that assumed a session would
        # take the whole surface down with it.
        canvas = MeasuringCanvas()
        self.assertEqual(ui._settings_row(canvas, mock.Mock(session=None),
                                          ui.BUBBLE_W, ui.PAD), 0)

    def test_the_ceiling_grew_by_the_strip_rather_than_the_draft_paying_for_it(self):
        """The strip is furniture, not content.

        Taking it out of the content's share is what the editor tests caught: the live
        partial's own cap is a flat 70 px, and on a panel pegged at the old ceiling it
        ran straight through the note and the chip row because everything above it had
        grown by 22 px and it had not been told.
        """
        self.assertEqual(ui.PANEL_MAX_H, 184 + ui.SETTINGS_H)
