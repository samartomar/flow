...
| P9 | **The conversation.** Converse mode is a spoken conversation with the local agent CLI, and it answers whatever is asked. Twice rewritten from use, and the second rewrite reversed the first. 2026-08-01 narrowed it to "discuss and refine prompts only, nothing more", because general conversation had failed at the desk — the CLI said it had no internet access, and hallucinated. 2026-08-03 found the cause and it was Flow's, not the model's: `WORKSHOP` rode the end of every question telling the CLI *not to carry out the task*, so asked "how are you", codex answered that the prompt was not an actionable coding task. It obeyed perfectly, which is the proof the instruction was wrong. With it gone the mode answers anything, so the scope is the wide one again. Questions are **grounded in a workspace** when one is set — an explicit project path, named at startup, on every mode switch, and at the moment of egress — and are answered ungrounded when it is not. **What is discussed can be kept**: a conversation leaves a document behind rather than dying on quit. | Two loops, and both are the acceptance test. **The prompt loop**: speak a rough prompt → the CLI's suggestions render in Flow → speak a follow-up, which continues the same conversation → **take the answer as the draft** (one chip, or "use that answer") → **send it** (one word, or one press) into the terminal where the work is. **The notes loop**: ask anything, for as long as it takes → **"keep note"** files the answer with the question that produced it (or one tap, right-click ▸ Keep this answer) → **"wrap up"** writes every kept note into one markdown file in the workspace, and onto the card where Copy takes it. With no workspace the notes stop at the card, because Flow chooses no folder on anybody's behalf. Nothing is written until both acts have happened. Switching modes is one action; the correction loop (P3) works on the outgoing prompt in both. Spoken replies via the OS speech engine, shipped. Half-duplex is a standing caveat, not a defect: there is no echo cancellation (R16), so Flow hears the user or talks, never both, and interrupting is an explicit action. |
