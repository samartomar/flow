...
- **Always local.** Microphone audio, the utterance buffers, the lexicon, the profile,
  ...
  code in those modules that could send anything anywhere. `flow --stats` is in this band
  too: it reads `~/.flow/diag.jsonl` and `~/.flow/profile.json`, prints a few lines and
  exits, and `flow/stats.py` has no opener in it to do anything else with them.
...
|---|---|---|---|
| main / UI | `Pill.mainloop()` | `Pill._tick()` every 30 ms → poll the foreground → `Session.tick()` → repaint; `Pill._fast_tick()` every 5 ms while a hold or a paste wait is in flight → drain the hotkeys, finish the paste | **The only thread that may touch Tk.** `_tick` re-schedules itself in a `finally`, so an exception cannot break the chain and leave a dead pill on screen. The foreground poll is two user-mode calls and is where `paste_target` comes from — asking at paste time asks after the click. The one thing that stops this thread is the right-click menu, which is a native `TrackPopupMenu` running its own modal loop |
...
provider and outcome category, and an utterance that reached the draft records how many
words it was and how long it took to say. It is a different thing from the event stream
and not a replacement for it: events are how the UI learns what to draw, the trace is what
is left after Flow is closed. See [§9](#9-what-is-written-to-disk) and `flow/diag.py`.

That last record is also the only one anything reads back. `flow --stats` counts the
`dictated` records dated today to answer "how much have I said", which is why the count
is written at `Session._remember_append` — the one seam every spoken word into the draft
passes through — rather than beside the route trace: a partial is replaced and never
arrives there, a local edit spends words the draft never gains, and a CLI rewrite returns
prose nobody spoke. `words` is on the allow-list for the reason the list exists at all:
an integer cannot be read back into a sentence, and every field in `diag.NEVER` can.

### The send word is heard by the partial, and fires on the silence after it

"boom" used to be a second utterance in full: the 800 ms hangover to close the gate,
then a final decode — ~0.8 s on this GPU for one word, the 30 s padding again — and only
then the router. Measured 1.6–2.5 s between the word and the paste. The partial decoder
was already hearing it at 0.7 s of audio, so `_pump_decodes` now hands every partial to
`_hear_trigger`: a partial that `edits.plan` reads as a whole send trigger, against a
draft that exists, inside `TRIGGER_MAX_SEC`, is remembered. It is *acted on* by
`_pump_audio` only once the open gate has counted `TRIGGER_QUIET_BLOCKS` of quiet — the
speaker has stopped, and "boom" was not the first syllable of a sentence about boom
boxes. Then the utterance is spent: no final, the gate reset, the trigger routed exactly
as the final would have routed it. Anything that ends the utterance first — the
hangover, a release, a pause — clears the memory, and the final decides as it always did.

### The release stops reading, not the stream

Under push-to-talk the release used to `mic.stop()` and the next press used to
`refresh()` and `start()`: `Pa_Terminate`, `Pa_Initialize`, a new `InputStream`, all on
the UI thread, and then 111–266 ms before the first block arrived — in front of the first
word, whose quiet head is what opens the gate. Now `talk_end` leaves the stream open and
stops reading it (`_mic_started` false, `_mic_lingering` true), and the next `start()`
inside `MIC_LINGER_SEC` drains what arrived unread, rebases the overflow counter so the
blocks the full queue threw away are not announced as lost, and captures from that
moment. `_pump_linger`, in `pump_results` so it runs armed or not, closes the stream a
minute after the last release. An open, unread stream is the posture the idle unload has
taken with the toggle gesture since the beginning — a device handle, not a recording —
and `tests/test_talk.py` pins both halves: the stream reuse and the close.
...
On a GPU the tiers are `small` for partials and `large-v3` for finals
(`asr.CUDA_PARTIAL_MODEL`, `asr.CUDA_MODEL`); see "The decode runs on the GPU" in §7 for
why that is no longer one model twice.
...
**A final makes the running partial stale.** `DecodeWorker` has one thread, and a final
submitted while a partial is being decoded used to wait for it — the release paid for the
partial and then for the final. `submit_final` now marks the partial stale; a transcriber
that says it is `cancellable` is asked between segments and stops, and a result that
finishes anyway is discarded rather than drawn after the final it previewed.
...
**Which collapsed the two-tier split, for a month.** `small.en` could not drive partials
because it cost 2.66–3.78 s per prefix against a 1.5 s budget — a CPU fact. On the GPU
`large-v3` costs 0.75 s at a 1 s prefix rising to 1.30 s at 13 s (median of three), so
`default_models("cuda")` returned the same model twice, retiring the visible
partial→final rewrite. That paragraph called 1.30 s against 1.5 s "the line to revisit
first if partials start arriving late", and the trace revisited it (2026-09-01): over
5 169 partials on this machine the median was **795 ms and the p90 1 357 ms** — inside
the budget and still a delayed caption rather than a live one, because Whisper pads
every input to 30 s and `large-v3`'s encoder costs the same for one word as for twenty.
The partial tier is `small` again (`CUDA_PARTIAL_MODEL`, multilingual so `translate`
still holds), an eighth of the encoder; the final is unchanged and is still what gets
pasted. The rewrite is back on a line that says it is provisional, and that is the trade.

Beam width is not where the final's time goes, measured the same day on the three
`.bench` reference clips: `large-v3` on this GPU decodes 2.0 s of audio in 764 ms at
beam 5 and 722 ms at beam 1, 9.9 s in 1 314 ms and 1 128 ms — the padded encoder is
the floor, and the ladder never fired on clean speech. `FINAL_BEAM` stays at 5.
...
| `speak.HOSTS` | `pwsh`, then `powershell` | Which PowerShell speaks. Not a style choice: `System.Speech` is a .NET API with two implementations that do not enumerate the same voices. Windows PowerShell 5.1 reads only the legacy SAPI5 token store; PowerShell 7 also reads OneCore. Measured: **3 voices under `powershell`, 9 under `pwsh`** on the same machine — 3 classic `TTS_MS_*_11.0` tokens plus 6 `MSTTS_V110_*`, so more of the 2013 generation, not better ones. Windows 11's natural voices are unreachable under either host: the MSIX package ships a valid SAPI token whose `categoryBase` (`Speech Server\v11.0`) is a hive that does not exist, and whose engine CLSID is registered in no COM store, so nothing enumerates it and nothing could instantiate it — WinRT `AllVoices` returns the same six OneCore voices (verified against a machine carrying AvaHD, Guy and Sonia). A better voice comes from `flow/piper.py`, not from here. The same executable must list *and* speak, or the menu offers names `SelectVoice` will quietly refuse |
...
Every line Flow needs a new user to know was a `print()`: the combos `RegisterHotKey`
actually accepted, the trigger word, the workshop, and — the load-bearing one — that a
pause sends a converse question. All of it goes to a console a GUI user does not have
open, and three outside users met the app without any of it (decisions.md 2026-08-03).

`help.welcome_rows()` is that content as rows, and `ui.HelpWindow` draws it: the same
window as the command sheet, with a different title and a `Dismiss` chip. A second
`Toplevel` would be a second set of the things this one has already been made to get
right — `WS_EX_NOACTIVATE`, a viewport with two ways to scroll, a row layout that cannot
clip — and the welcome card needs every one of them. Generated for the same reason the
sheet is: a welcome naming `ctrl+alt+space` would greet somebody with a key that was
already taken on the machine this was built on.

Shown once, gated on `profile.welcomed`, and **the flag is written before the card is
shown** — a crash between showing and saving would show it twice, which is the one
outcome worse than showing it once. `--no-profile` gets no card at all: there is nowhere
to remember it, and a welcome on every launch is an advertisement rather than an
introduction. Absent reads as *not welcomed*, so an upgrade shows it once — somebody who
has used Flow for a week still has not been told what the colours mean.

**The colour legend is in the Help sheet permanently**, not only on the card, because a
legend is exactly the thing somebody wants a second time. It describes the palette item
63 left behind: three pill colours plus the error flash, and the two that departed as
window identities. That growth is why `HELP_MAX_H` moved from 1040 to 1190 — the sheet
measures **1174 px** with every combo registered, so it has outgrown a 1200-tall desktop
and scrolls there. Recorded rather than fixed by trimming the legend: the content is what
somebody came for.
...
| unit tests ↻ | `uv run python -m unittest discover -s tests` | **1393 passed**, 39.4 s (2026-08-04, round ten's close; the row read 437, then 766, then 1251, for long enough each time to be worth saying out loud — the count is re-read here whenever the suite is) |
