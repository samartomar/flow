...
| `ASK_ARTIFACT_MAX_CHARS` | 12 000 | The artifact render bound — a bound, not a brief: truncating a prompt someone asked for in full is worse than a tall bubble. This row used to say "the bubble scrolls", and on 2026-08-02 that was measured and is not true of any path: 12 000 characters sized the window **4 179 px** on a 672 px desktop and it was simply placed off the bottom, chips and all. What is true now is that the window stops at the desktop edge (`EDGE_AIR`, §7) and the tail past that is clipped rather than off-screen; a reply that genuinely scrolls is a NEEDS_YOU proposal. The spoken half flips instead: past `ARTIFACT_SAY_MAX_LINES` / `ARTIFACT_SAY_MAX_CHARS` the voice says only "a 12-line answer is on screen", because invariant 6 makes a read-aloud artifact minutes of deafness |
