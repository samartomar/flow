# Loop rounds 1–3 — closing summaries, the round-three queue, and items 0–25

Archived 2026-08-01 from LOOP_PLAN.md when the queue emptied. This is the record of
three loop rounds plus the two-item micro-round: what each item did, its before/after
evidence, and the round summaries. The Rules and the live queue stay in
[../../LOOP_PLAN.md](../../LOOP_PLAN.md); the decisions behind these items are in
...
**Closed after the fact, same day:** item 24 gave `_provider()` the pin, so the notes name
the CLI the call will actually make — `a18b619`, suite 766 → 773, selfdrive 64/64.
Item 25 caught README.md up to all nine of the round's features — `ca38918`, eight
feature greps 0/8 before and 8/8 after, and the "nothing leaves the machine" line retired.
...
- Doc sync: §10's provider-gap closure sentence gains "and the words obey the pin";
  nothing else moved.
- Status: done — commit a18b619, 2026-08-01
- Evidence: new `TestTheNotesObeyThePin` in `tests/test_converse.py`, 7 checks (one a
  3-case subTest). **Before:** 6 failed / 1 passed — with codex and claude both
  resolvable and claude pinned, the mode-switch note read "Ask sends the draft to
  codex", `_start_ask` emitted `asking codex… | answered via claude` and `_start_refine`
  `refining via codex: 'make it formal' | refined via claude` — the warning and its own
  receipt naming different CLIs — and `_provider()` returned `codex` where
  `Pill._marker()` returned `claude` from the same inputs. The one that passed is the
  control: unpinned still takes `available()[0]`. **After:** 7/7 green, control
  included. The fix reads `self._cli` first and only walks the order when it is None,
  the shape `Pill._marker` already had; a pin that has left PATH is still named, because
  `_invoke_any` never re-checks PATH for an explicit `cli=` — that pin is what runs and
  what fails, and a note is not the place to invent a fallback the caller does not have.
  Suite **766 → 773, OK, 15.9 s**; `scripts/selfdrive.py` **64/64** first run, no rerun.
  Doc sync: §10's closure sentence gains the pin clause. One process note: the commit
  landed with a shell-quoting artifact for a subject (`@`) and its message — no files —
  was amended immediately, against Rule 3's "never amend", to satisfy the same rule's
  "plain imperative subject line". Unpushed, seconds old, and the choice is recorded
  here rather than left in the log as `f83732e @`.

### 25. Catch the README up to round three
Owner-asked 2026-08-01 ("is read me file updated?" — it is not: last touched at
`2732c5a`, before the round began, and none of the round's nine user-visible features
appears in it). Same drift as the item-13 README entry the owner closed by hand, same
structural cause: Rule 6 syncs `docs/architecture.md` and README.md is never a named
file. Mechanical, no behaviour, one commit.
- Files: `README.md` only.
- Do: read the file end to end against the shipped build (architecture.md is current —
  round three synced it every item), and bring every section that describes a surface
  the round changed up to what ships. The features that must each appear where the
  README describes that surface, with their real words:
  1. the converse marker reads `codex`/`claude` on the pill (item 15);
  2. the draft is editable by hand — the Edit chip, mic suspended while the editor has
     focus, Enter inserts a newline, Send after an edit still aims at the tracked
     window (item 17);
  3. the right-click menu offers a twice-seen inferred pair, one tap writes the arrow
     line — and the lexicon contract changed: Flow appends one line on an explicit tap,
     never edits or removes (item 19; the §9 rewrite is the source of truth);
  4. "follow and …" routes as a follow-up; bare "follow" is still dictation (item 20);
  5. a reply can become the draft — the chip and the spoken forms "use that answer" /
     "use that reply", replace-not-append, flips to dictate (item 21);
  6. the spoken send triggers — "boom" pastes, "enter boom" pastes and submits; whole
     utterance only; words configurable in `profile.json` (item 22);
  7. converse mode is a prompt workshop grounded in a workspace — the `--cwd` flag, the
     `workspace` profile field, the startup "workshop: …" line, and what the preamble
     sends (item 23; the README's "what leaves the machine" wording must match the
     architecture doc's updated paragraph);
  8. the trace records a per-route `confidence`, and every bench result carries an
     identity block (items 16, 18 — one sentence each where the README mentions the
     trace and the benchmarks).
- Instrument first, docs-style (item 0's precedent): the eight greps above as absence
  checks before, presence checks after; the unit suite run before and after as the
  baseline record (docs cannot break it; the run is the evidence the commit gates on).
- Guard: nothing in the README may promise more than the build — where a feature has a
  bound (trigger words need an editor to change; the workshop path can go stale), the
  README states the bound the way the architecture doc does. The last catch-up's
  finding applies: check the storage/writes table against §9 line by line.
- Acceptance: suite green; all eight presence greps pass; no section describes the
  pre-round behaviour of a surface the round changed.
- Doc sync: none — this item *is* the sync.
- Status: done — commit ca38918, 2026-08-01
- Evidence: eight greps, one per feature, 28 patterns between them, run against a
  whitespace-flattened README. **Before: 0/8 checks, 1/28 patterns** — the only hit was
  the phrase "whole utterance", already there for another reason. **After: 8/8, 28/28.**
  Suite **773, OK, 15.6 s** before and after, as expected of a docs item: the run is the
  baseline record, not the instrument. What was actually stale: the pill's **ASK** badge
  where the marker now names the CLI; no Edit chip, no Use this, no spoken send, no
  `--cwd`; a startup block missing three lines the app prints and quoting a fourth
  ("fallbacks: claude") the app no longer does; the storage table missing `diag.jsonl`
  entirely, which is where the per-route `confidence` had to go; and "Nothing leaves the
  machine" on a page whose Ask now sends a filesystem path — replaced with the
  architecture doc's data-boundary wording. The §9 line-by-line found the missing trace
  row and three profile fields. The Guard also caught two promises the build does not
  keep: "never presses Enter for you" (bounded now by the trigger that does) and the flat
  three-sentence answer cap (an artifact request lifts it). No behaviour changed and
  `README.md` was the only file in the commit.
